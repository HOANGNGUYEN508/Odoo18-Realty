import { Component, useState, onWillStart, useEffect, useRef, onMounted, onWillUnmount } from "@odoo/owl";
...
export class FilterDialog extends Component {
	...
	setup() {
		...
		this.values = useState({
			...
			feature_ids:        Array.isArray(initial.feature_ids) ? initial.feature_ids : [],
		...
		const handleClickOutside = (event) => {
			if (this.state.dropdownOpen && this.containerRef.el && !this.containerRef.el.contains(event.target))
		...
		onWillStart(async () => {
			...
			try {
				// Load dropdown options and saved filters
				const models = [
					["region", "regions", ["id", "name"]],
					["res.country.state", "provinces", ["id", "name"]],
				...
				const results = await Promise.all(
					models.map(([model, , fields]) =>
						this.orm.call(model, "search_read", [
							[["active", "=", true]],
							fields,
						])
					)
				...
				models.forEach(([, key], i) => {
					this.options[key] = [{ id: "", name: labels[key] }, ...results[i]];
				});
