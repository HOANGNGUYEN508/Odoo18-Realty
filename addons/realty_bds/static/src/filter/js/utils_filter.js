export function parseDomainToValues(rawDomain, rawContext) {
    ...
    const v = {
        province_id: "", district_id: "", commune_id: "", region_id: "",
        status_id: "", type_id: "", land_tittle_id: "", feature_ids: [],
        min_price: "", max_price: "", unit_price_min_id: "", unit_price_max_id: "",
...
export function buildDomainFromValues(states, options) {
    ...
    const simpleFields = [
        'province_id', 'district_id', 'commune_id', 'region_id',
        'status_id', 'type_id', 'land_tittle_id',
    ];
    ...
    const fetchMultiplier = unitId => {
        ...
        if (!uid) return 1;
        const unit = options.unit_price_units.find(u => u.id === uid);
    ...
    if (absMin != null || absMax != null) {
        ...
        if (!isNaN(minP)) domain.push(['absolute_price','>=', minP * fetchMultiplier(states.unit_price_min_id)]);
        if (!isNaN(maxP)) domain.push(['absolute_price','<=', maxP * fetchMultiplier(states.unit_price_max_id)]);
    }
