...
publicWidget.registry.SignupForm = publicWidget.Widget.extend({
    ...
    _onDistrictChange: async function (ev) {
        ...
        communeSelect.innerHTML = '<option value="" selected>Select Commune</option>'; 
    ...
    _onProvinceResidentChange: async function (ev) {
        ...
        let district_residentSelect = document.querySelector("#district_resident_id");
        ...
        district_residentSelect.innerHTML = '<option value="" selected>Select District</option>'; 

        try {
            let result = await rpc("/get_districts", { province_id: province_residentId });  
            if (result.districts.length === 0) {
                console.warn("⚠️ No districts found for Province ID:", province_residentId);
            } else {
                result.districts.forEach(d => {
                    let option = document.createElement("option");
                    option.value = d.id;
    ...
    _onDistrictResidentChange: async function (ev) {
        ...
        commune_residentSelect.innerHTML = '<option value="" selected>Select Commune</option>'; 

        try {
            let result = await rpc("/get_communes", { district_id: district_residentId });  
            if (result.communes.length === 0) {
                console.warn("⚠️ No communes found for District ID:", district_residentId);
            } else {
                result.communes.forEach(c => {
                    let option = document.createElement("option");
                    option.value = c.id;
