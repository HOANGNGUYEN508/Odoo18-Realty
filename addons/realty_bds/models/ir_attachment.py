...
class IrAttachment(models.Model):
    ...
    orphaned_from_model = fields.Char(string="Orphaned From Model",
                                      readonly=True)
        ...
        for att in attachments:
            try:
                ...
                if att.create_uid.id == current_uid:
                    ...
                    raise AccessError(
                        "You are not allowed to mark attachment %s as orphaned.",
                        att.id)
    ...
    def mark_true(self, ids):
        ...
        valid_ids = [
            att_id for att_id in ids if isinstance(att_id, int) and att_id > 0
        ]
        if not valid_ids: return True
