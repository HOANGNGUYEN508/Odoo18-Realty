...
class ResPartner(models.Model):
    ...
    province_id = fields.Many2one('res.country.state', string="Province/City", ondelete='restrict', tracking=True)
    ...
    province_resident_id = fields.Many2one('res.country.state', string="Residence Province/City ", ondelete='restrict', tracking=True)
    ...
    def _check_citizen_id(self):
        for record in self:
            ...
            if not record.citizen_id:
                raise ValidationError("❌ Citizen ID number is required for individuals!")
    ...
    def _check_location_consistency1(self):
        for record in self:
            if record.is_company or not record.user_ids:  # Ignore companies or partners not linked to any user
                continue
    ...
    def _onchange_province_resident_id(self):
        if self.province_resident_id:
            if self.district_resident_id and self.district_resident_id.province_id != self.province_resident_id:
                self.district_resident_id = False
            if self.commune_resident_id:
                self.commune_resident_id = False
            return {'domain': {'district_resident_id': [('province_id', '=', self.province_resident_id.id)]}}
        else:
            self.district_resident_id = False
            self.commune_resident_id = False
    ...
    def _onchange_district_resident_id(self):
        if self.district_resident_id:
            if self.commune_resident_id and self.commune_resident_id.district_id != self.district_resident_id:
                self.commune_resident_id = False
            return {'domain': {'commune_resident_id': [('district_id', '=', self.district_resident_id.id)]}}
        else:
            self.commune_resident_id = False
    ...
    def action_notify_admin_new_partner(self, partner):
        ...
        users_to_notify = self.env['res.users'].search([('groups_id', 'in', [group.id])])
