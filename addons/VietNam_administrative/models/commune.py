...
class Commune(models.Model):
    ...
    #Constrain
    _sql_constraints = [
        ('unique_name', 'UNIQUE(name,district_id,province_id)', 'This name already exists!'),  # Ensure uniqueness
    ]
    ...
    def _check_district_province(self):
        for record in self:
            ...
            if record.district_id and record.province_id and record.district_id.province_id != record.province_id:
                raise ValidationError(f"❌ Error: District '{record.district_id.name}' does not belong to province '{record.province_id.name}'!")
    ...
    def _onchange_province_id(self):
        ...
        if self.province_id:
            ...
            self.district_id = False
        return {'domain': {'district_id': [('province_id', '=', self.province_id.id)] if self.province_id else []}}  # Do not display districts if no province is selected